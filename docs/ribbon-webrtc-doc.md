...
than the object retrieved from the [getConfig][15] API. This may happen when a format
...
provide are as described by the [config][16] section.
...
*   `customParameters` **[Array][13]<[call.CustomParameter][22]>** The locally set Custom Parameters for the call.
...
It represents an RTCPeerConfiguration dictionary, whose parameters are documented [here][29].
...
*   `bundlePolicy` **[string][8]?** For further description on this and other properties,
    see [RTCPeerConnection's configuration parameters][29].
...
This object is provided to the [IceCollectionCheckFunction][32], and contains the
...
*   `reason` **[string][8]** The reason the check function was called. Three possible values:
    ...
    'Scheduled' - A scheduled call (for first invocation, and subsequent invocations based on `wait` value returned by [IceCollectionCheckFunction][32].)
...
*   `iceGatheringState` **[string][8]** The current ICE gathering state.
    See [RTCPeerConnection.iceGatheringState][33].
...
([IceCollectionInfo][34]), which it can use to dictate how to proceed with a call.
...
The function must then return an appropriate result object in the format of [IceCollectionCheckResult][35]
...
*   `tracks` **[Array][13]<[call.TrackObject][40]>** A list of Track objects that are contained in this Media object.
...
*   `camera` **[Array][13]<[call.DeviceInfo][41]>** A list of camera device information.
*   `microphone` **[Array][13]<[call.DeviceInfo][41]>** A list of microphone device information.
*   `speaker` **[Array][13]<[call.DeviceInfo][41]>** A list of speaker device information.
...
These parameters can be specified with the [call.make][42] and [call.answer][43] APIs.
They can also be set after a Call is established using the [call.setCustomParameters][44] API, and sent using the [call.sendCustomParameters][45] API.
...
When these custom headers are received, the SDK will emit a [call:customParameters][46] event
which will contain the custom parameters that were received.

A Call's custom parameters are stored on the Call's [CallObject][47],
which can be retrieved using the [call.getById][24] or
[call.getAll][23] APIs. These are the parameters that will be sent to the remote
endpoint of the Call. Parameters received from a Call are not stored as
part of the CallObject and are only provided via the [call:customParameters][46] event.
...
[call.addMedia][54] API can be used.

The progress of the operation will be tracked via the
[call:operation][49] event.

The SDK will emit a [call:tracksAdded][55]
...
The SDK will emit a [call:tracksRemoved][57]
event for both the local and remote users to indicate that a track
has been removed.
...
The [call.addMedia][54] API can be used to perform the same behaviour
as `startScreenshare`. [call.addMedia][54] is a general-purpose API
...
[call.addMedia][54] will perform the same behaviour as using
`startScreenshare`.
...
the Call, such as local video or remote audio, can be [muted][61]
or [unrendered][62] during this operation if desired.

This operation will use the browser's
[Audio constructor][63]
...
The SDK will emit [call:operation][49] events locally
...
[media:trackEnded][64] event will be emitted
...
the [call.replaceTrack][65] API to revert the local audio track.
...
Returns **[Promise][66]** Promise that resolves when the operation is complete.
...
Returns **[Array][13]<[call.CallObject][67]>** Call objects.
...
Returns **[call.CallObject][67]** A call object.

### setCustomParameters

Set the [Custom Parameters][68] of a Call, to be provided to the remote endpoint.
...
A Call's custom parameters are a property of the Call's [CallObject][47],
which can be retrieved using the [call.getById][24] or
[call.getAll][23] APIs.

The custom parameters set on a call can be sent directly with the [call.sendCustomParameters][45] API.
...
To change or remove the custom parameters on a call, use the [call.setCustomParameters][44] API.
...
The data retrieved is a [RTCStatsReport][69]
...
The SDK will emit a
[call:statsReceived][71] event, after
...
Returns **[Promise][66]** A promise that will resolve with the stats report or an error if it fails.
...
In addition, the SDK emits a [call:availableCodecs][72] event
upon retrieving that list of codecs.

This API is a wrapper for the static method [RTCRtpSender.getCapabilities()][73].
...
See event documentation [here][74].
See metrics documentation [here][75].
...
*   `sdpHandlers` **[Array][13]<[call.SdpHandlerFunction][77]>** The list of SDP handler functions to modify SDP.

Returns **[undefined][78]** 
...
re-rendering it with a new speaker with [media.renderTracks][79].
...
problem. Using the [call.restartMedia][80] API will attempt to reconnect the media. See
the [call.restartMedia][80] API description for more information.

These states are direct reflections of the possible
[RTCPeerConnection.iceConnectionState][81] values.

The Call's media connection state is a property of the [CallObject][47],
which can be retrieved using the [call.getById][24] or
[call.getAll][23] APIs.

The SDK emits a [call:mediaConnectionChange][59]
...
*   `FAILED` **[string][8]** The connection has failed and cannot be recovered automatically. A full media connection refresh is required to reestablish a connection. See the [call.restartMedia][80] API.
...
The call report can be retrieved via the [call.getReport][82] API.
...
information (see the [call.getById][24] API). This event indicates that
...
Information about the Call can be retrieved using the [call.getById][24]
API.
...
to the Call, using the [call.addMedia][54] API for example, or when the
...
Information about a Track can be retrieved using the [media.getTrackById][83] API.
...
tracks, by using the [call.removeMedia][56] API for example, or when the
Call is held with the [call.hold][84] API.
...
See the [call.getStats][85] API for more information.
...
*   `params` **[Object][7]** 
    ...
    *   `params.result` **[Map][86]** The RTCStatsReport.
    *   `params.error` **[api.BasicError][19]?** An error object, if the operation was not successful.
...
A local Track has been replaced by the [call.replaceTrack][65] API.
...
available, similar to the [call:tracksRemoved][57]
event, and the new Track is available in its place, similar to the
[call:tracksAdded][55] event. The event
...
This event is emitted as a result of the [call.getAvailableCodecs][87] API. Please refer to the API for more
information.
...
To check the media connection state of a call, retrieve the call's information using the  [call.getById][24] API,
and check the `mediaConnectionState` property of the call.
See [call.mediaConnectionStates][26] for the list of possible values and descriptions.
...
This event is emitted as a result of the [call.restartMedia][80] API being called.
See the description for [call.restartMedia][80] for information about its
usage.

The [call:mediaConnectionChange][59] event
...
Can be retrieved using the [connection.getSocketState][88] API.
...
Returns **[connection.WSConnectionObject][89]** Details about the current websocket connection, including state and configuration.
...
[config.logs][90]) to customize how the logs are
...
[config.logs][90]) will need to handle LogEntry
objects.
...
[config.logs][90]). The SDK will then provide this
...
SDK has access to. Media is used in conjunction with the [Calls][91]
...
the [media.getDevices][92] API.

### getDevices

Retrieves the available media devices for use.

The [devices:change][93] event will be
...
Returns **[call.MediaObject][94]** A Media object.
...
For device information, the [media.getDevices][92] API will retrieve
...
recommend using the [media.initializeDevices][95] API in this
...
The SDK will emit a [devices:change][93]
event when the operation is successful or a
[devices:error][96] event if an error is
encountered.
...
The SDK will emit a [media:muted][97] event
...
The SDK will emit a [media:unmuted][98] event
...
A Track can be muted using the [media.muteTracks][61] API.
...
A Track can be unmuted using the [media.unmuteTracks][100]
API.
...
[media:sourceMuted][102] event), and will
...
[call.removeMedia][56] API so the remote side of the Call knows the track
has stopped, or the track can be replaced with a new track using the
[call.replaceTrack][65] API to prevent any interruption.
...
standard and/or voip push notifications. It can then send these notifications to the SDK with [api.notifications.process][103]
...
Returns **[Promise][66]** When successful,  the information of the registration.
...
Returns **[Promise][66]** When successful, the promise will resolve with undefined.
Promise will reject with error object otherwise.
...
This is a wrapper on top of the browser's [fetch API][104]
...
*   `init` **RequestInit** An object containing any custom settings that you want to apply to the request. See [fetch API][104]
...
Returns **[Promise][66]<[Response][105]>** A promise for a [Response][106] object.
...
initialization of the SDK using the [config.call.sdpHandlers][107] configuration
parameter. They can also be set after the SDK's creation by using the
[call.setSdpHandlers][108] function.
...
removal handler in combination with the [call.getAvailableCodecs][87] function in order to build logic to determine the best codecs to use
...
Returns **[call.SdpHandlerFunction][77]** The resulting SDP handler that will remove the codec.
...
[29]: https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection/RTCPeerConnection#parameters
...
[32]: #callicecollectioncheckfunction

[33]: https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection/iceGatheringState

[34]: #callicecollectioninfo

[35]: #callicecollectioncheckresult

[36]: #callicecollectioninfo

[37]: config#config.call
...
[44]: #callsetcustomparameters

[45]: #callsendcustomparameters

[46]: call.event:call:customParameters

[47]: #callcallobject

[48]: call.event:call:trackEnded
...
[52]: call.event:call:newTrack

[53]: #calldscpcontrols

[54]: #calladdmedia

[55]: #calleventcalltracksadded

[56]: #callremovemedia

[57]: #calleventcalltracksremoved

[58]: #calleventcalltrackreplaced

[59]: #calleventcallmediaconnectionchange

[60]: #calleventcallmediarestart

[61]: #mediamutetracks

[62]: #mediaremovetracks

[63]: https://developer.mozilla.org/en-US/docs/Web/API/HTMLAudioElement/Audio

[64]: #mediaeventmediatrackended
...
[68]: #callcustomparameter

[69]: https://developer.mozilla.org/en-US/docs/Web/API/RTCStatsReport

[70]: https://developer.mozilla.org/en-US/docs/Web/API/RTCStats

[71]: #calleventcallstatsreceived

[72]: #calleventcallavailablecodecs

[73]: https://w3c.github.io/webrtc-pc/#dom-rtcrtpsender-getcapabilities
...
[79]: #mediarendertracks

[80]: #callrestartmedia

[81]: https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection/iceConnectionState

[82]: #callgetreport

[83]: #mediagettrackbyid

[84]: #callhold

[85]: #callgetstats

[86]: https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Map

[87]: #callgetavailablecodecs

[88]: #connectiongetsocketstate

[89]: #connectionwsconnectionobject
...
[93]: #mediaeventdeviceschange

[94]: #callmediaobject

[95]: #mediainitializedevices

[96]: #mediaeventdeviceserror

[97]: #mediaeventmediamuted

[98]: #mediaeventmediaunmuted

[99]: #callmediaconstraint

[100]: #mediaunmutetracks

[101]: #mediaeventmediasourceunmuted

[102]: #mediaeventmediasourcemuted

[103]: api.notifications.process

[104]: https://developer.mozilla.org/en-US/docs/Web/API/fetch

[105]: https://developer.mozilla.org/docs/Web/Guide/HTML/HTML5

[106]: https://developer.mozilla.org/en-US/docs/Web/API/Response

[107]: #configconfigcall

[108]: #callsetsdphandlers
