...
The SDK will emit a [call:tracksAdded][60] event
both for the local and remote users to indicate a track has been
added to the Call.
...
The SDK will emit a [call:tracksRemoved][62]
event for both the local and remote users to indicate that a track
has been removed.
...
the Call, such as local video or remote audio, can be [muted][69]
or [unrendered][70] during this operation if desired.

This operation will use the browser's
[Audio constructor][71]
...
[media:trackEnded][72] event will be emitted
...
the [call.replaceTrack][73] API to revert the local audio track.
...
Returns **[Promise][74]** Promise that resolves when the operation is complete.
...
The SDK emits a [call:mediaConnectionChange][78]
...
Information about a Track can be retrieved using the [media.getTrackById][81] API.
...
tracks, by using the [call.removeMedia][61] API for example, or when the
Call is held with the [call.hold][82] API.
...
See the [call.getStats][83] API for more information.
...
*   `params` **[Object][7]** 
    ...
    *   `params.result` **[Map][84]** The RTCStatsReport.
    *   `params.error` **[api.BasicError][27]?** An error object, if the operation was not successful.
...
available, similar to the [call:tracksRemoved][62]
event, and the new Track is available in its place, similar to the
[call:tracksAdded][60] event. The event
...
The [call:mediaConnectionChange][78] event
...
A Track can be muted using the [media.muteTracks][69] API.
...
[call.removeMedia][61] API so the remote side of the Call knows the track
...
Returns **[Promise][74]<[Response][103]>** A promise for a [Response][104] object.
...
[60]: #calleventcalltracksadded

[61]: #callremovemedia

[62]: #calleventcalltracksremoved
...
[71]: https://developer.mozilla.org/en-US/docs/Web/API/HTMLAudioElement/Audio

[72]: #mediaeventmediatrackended
...
[78]: #calleventcallmediaconnectionchange

[79]: #calleventcallmediarestart

[80]: #mediarendertracks

[81]: #mediagettrackbyid

[82]: #callhold

[83]: #callgetstats

[84]: https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Map
